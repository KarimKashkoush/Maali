...
type Frame = { frame_width: number; frame_height: number; detections: Face[] };
...
function AttendanceBoard({ id }: { id: number }) {
  ...
  function clearCamera() {
    ...
    if (videoRef.current) videoRef.current.srcObject = null; setFaces([]);
  }
  ...
  async function capture(session: number, generation: number) {
    ...
    try {
      if (video && canvas && video.videoWidth) {
        const scale = Math.min(1, 1920 / Math.max(video.videoWidth, video.videoHeight)); canvas.width = Math.round(video.videoWidth * scale); canvas.height = Math.round(video.videoHeight * scale);
        ...
        if (blob) {
          ...
          if (generation !== generationRef.current) return;
          setSize({ width: result.frame_width || canvas.width, height: result.frame_height || canvas.height }); setFaces(result.detections ?? []);
        }
      }
    ...
    if (generation === generationRef.current) timerRef.current = setTimeout(() => void capture(session, generation), 900);
  }
  async function start() {
    ...
    try {
      ...
      setSize({ width: videoRef.current.videoWidth || 16, height: videoRef.current.videoHeight || 9 }); setState("running"); await refresh(); void capture(created, generation);
    } catch (e) {
      if (generation !== generationRef.current) return;
